...
export const TenantDetailsModal: React.FC<TenantDetailsModalProps> = ({
  ...
  // Track expanded/collapsed state for each section - all collapsed by default
    ...
    <>
      ...
      >
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3 flex-1">
              ...
              <div className="flex-1">
                <h2 className="text-lg font-semibold text-gray-900">{editedTenant.name}</h2>
        ...
        <div className="px-6 py-6 space-y-5">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      />
                      {editedTenant.depositWeeks && editedTenant.monthlyRent && (
                        <div className="absolute right-3 top-2 text-xs text-gray-400">Auto</div>
                      )}
                    </div>
                  </div>
                  <div>
          ...
          {/* Credit Checks Section - Always show */}
          <div className="bg-white border border-gray-200 rounded-lg p-5">
            <button
              type="button"
              onClick={() => toggleSection('creditChecks')}
              className="w-full flex items-center justify-between mb-4 hover:opacity-80 transition-opacity"
            >
              <div className="flex items-center space-x-2">
                <ShieldCheckIcon className="w-5 h-5 text-blue-600" />
                <h3 className="text-base font-semibold text-gray-900">Credit Checks</h3>
                {hasOnboardingData && onboardingData.creditChecks && onboardingData.creditChecks.length > 0 && (
                  <span className="text-xs text-gray-500 bg-gray-100 px-2 py-1 rounded ml-2">
                    {onboardingData.creditChecks.length}
                  </span>
                )}
              </div>
              {expandedSections.creditChecks ? (
                <ChevronUpIcon className="w-5 h-5 text-gray-500" />
              ) : (
                <ChevronDownIcon className="w-5 h-5 text-gray-500" />
              )}
              ...
              <div className="space-y-4 transition-all duration-200 ease-in-out">
                {hasOnboardingData && onboardingData.creditChecks && onboardingData.creditChecks.length > 0 ? (
                  ...
                  <div className="text-center py-8">
                    ...
                    <p className="text-sm text-gray-500">No credit checks yet</p>
                    <p className="text-xs text-gray-400 mt-1">Credit check information will appear here once completed</p>
                  </div>
                )}
              </div>
            )}
              ...
              <div className="space-y-4 transition-all duration-200 ease-in-out">
                <div className="text-center py-8">
                  ...
                  <p className="text-xs text-gray-400 mt-3">
                    Upload tenancy agreements, right to rent, deposit certificates, etc.
                  </p>
                </div>
              </div>
            )}
          ...
          {hasOnboardingData && onboardingData.tenancyAgreement && (
            <div className="bg-white border border-gray-200 rounded-lg p-5">
              <button
                type="button"
                onClick={() => toggleSection('tenancyAgreement')}
                className="w-full flex items-center justify-between mb-4 hover:opacity-80 transition-opacity"
              >
                <div className="flex items-center space-x-2">
                  <DocumentTextIcon className="w-5 h-5 text-blue-600" />
                  <h3 className="text-base font-semibold text-gray-900">Tenancy Agreement</h3>
                  <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                    onboardingData.tenancyAgreement.status === 'signed' 
                      ? 'bg-green-100 text-green-800' 
                      : 'bg-yellow-100 text-yellow-800'
                  }`}>
                    {onboardingData.tenancyAgreement.status === 'signed' ? '✓ Signed' : 'Pending'}
                  </span>
                </div>
                {expandedSections.tenancyAgreement ? (
                  <ChevronUpIcon className="w-5 h-5 text-gray-500" />
                ) : (
                  <ChevronDownIcon className="w-5 h-5 text-gray-500" />
                )}
          ...
          {hasOnboardingData && onboardingData.preparation && (
            <div className="bg-white border border-gray-200 rounded-lg p-5">
              <button
                type="button"
              ...
              >
                <div className="flex items-center space-x-2">
                  ...
                  <h3 className="text-base font-semibold text-gray-900">Preparation Checklist</h3>
                  {onboardingData.preparation.checklist && (
                    <span className="text-xs text-gray-500 bg-gray-100 px-2 py-1 rounded">
                      {onboardingData.preparation.checklist.filter((item: any) => item.completed).length}/{onboardingData.preparation.checklist.length}
                    </span>
                  )}
                </div>
              ...
              {expandedSections.preparation && (
                <div className="space-y-4 transition-all duration-200 ease-in-out">
                  ...
                  {onboardingData.preparation.checklist && onboardingData.preparation.checklist.length > 0 ? (
                    onboardingData.preparation.checklist.map((item: any, index: number) => (
