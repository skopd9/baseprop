...
import { LeaseParserModal } from './LeaseParserModal';
import { LeaseParserService } from '../services/LeaseParserService';
import { ParsedLeaseData, FieldConfidence } from '../types/leaseParsing';
...
export const SimplifiedAddTenantModal: React.FC<SimplifiedAddTenantModalProps> = ({
  ...
  // Lease parser state
  const [showLeaseParser, setShowLeaseParser] = useState(false);
  const [leaseDataApplied, setLeaseDataApplied] = useState(false);
  ...
  // Handle extracted lease data
  const handleLeaseDataExtracted = (data: ParsedLeaseData, confidence: Record<string, FieldConfidence>) => {
    const formMapping = LeaseParserService.mapToTenantForm(data);
    
    setFormData(prev => ({
      ...prev,
      firstName: formMapping.firstName || prev.firstName,
      surname: formMapping.surname || prev.surname,
      email: formMapping.email || prev.email,
      phone: formMapping.phone || prev.phone,
    }));

    // Update tenancy prep data if we have lease dates and rent info
    if (formMapping.leaseStartDate || formMapping.leaseEndDate || formMapping.monthlyRent || formMapping.depositAmount) {
      setTenancyPrepData(prev => ({
        ...prev,
        startDate: formMapping.leaseStartDate || prev.startDate,
        endDate: formMapping.leaseEndDate || prev.endDate,
        monthlyRent: formMapping.monthlyRent || prev.monthlyRent,
        deposit: formMapping.depositAmount || prev.deposit,
      }));
    }

    setLeaseDataApplied(true);
    setShowLeaseParser(false);
  };
  ...
  return (
    ...
    >
            ...
            {currentWizardStep === 0 && (
              <div className="space-y-4">
                <div className="mb-4 flex items-start justify-between">
                  ...
                  <button
                    type="button"
                    onClick={() => setShowLeaseParser(true)}
                ...
                {/* Lease Data Applied Indicator */}
                {leaseDataApplied && (
                  <div className="mb-4 p-3 bg-emerald-50 border border-emerald-200 rounded-lg flex items-center gap-2">
                    <CheckCircleIcon className="w-5 h-5 text-emerald-600" />
                    <p className="text-sm text-emerald-700">
                      Form pre-filled from lease document. Please review the extracted values.
                    </p>
                  </div>
                )}
      ...
      {/* Lease Parser Modal */}
      <LeaseParserModal
        isOpen={showLeaseParser}
        onClose={() => setShowLeaseParser(false)}
        onDataExtracted={handleLeaseDataExtracted}
        mode="tenant-form"
      />
    </div>
  );
