...
- **Supabase credentials required**: The app throws at runtime if `VITE_SUPABASE_URL` and `VITE_SUPABASE_ANON_KEY` are not set. A `.env` file with placeholder values allows build/lint/dev-server to start, but auth and data flows require real Supabase project credentials.
